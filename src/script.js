const component6 = document.getElementById('comp6')
const component5 = document.getElementById('comp5')
const component4 = document.getElementById('comp4')
const component3 = document.getElementById('comp3')
...
function checkSpaceSmaller(comp1){
    const space = window.innerWidth - comp1.offsetLeft
    if (space < 270){
        showMore.style.display = "block";
        dropAdds.appendChild(comp1)
...
function checkSpaceBigger(comp1, comp2){
    const postSpace = window.innerWidth - comp2.offsetLeft
    if (postSpace >=280 ) {
        mainMenu.insertBefore(comp1, showMore)
        arrayComponents.unshift(arrayRemove.pop())
        lastX = window.innerWidth
    }
    else if(dropAdds.childElementCount === 1 && postSpace >180){
        mainMenu.appendChild(comp1)
        showMore.style.display = "none";
    }
}

function nextMove () {
    let cuurentWindowWidth = window.innerWidth
    if(cuurentWindowWidth < lastX){
        if (arrayComponents.length > 1 ){
            let currentComp = arrayComponents[0]
            checkSpaceSmaller(currentComp)
        }
    }
    else if(cuurentWindowWidth > lastX){
        if (arrayRemove.length > 0 ){
            let len = arrayRemove.length-1
            let toDisplay = arrayRemove[len]
            console.log(len, toDisplay)
            let displayedComp = arrayComponents[0]
